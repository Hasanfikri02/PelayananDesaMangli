    ...
    <div class="flex min-h-screen bg-gray-900 text-gray-100 font-poppins">
        ...
        <main class="flex-1 p-8 overflow-y-auto bg-gray-900 rounded-tl-3xl shadow-inner">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-yellow-400">📄 Daftar Jenis Surat</h1>
                    <p class="text-gray-300 text-sm">Kelola seluruh jenis surat administrasi desa di sini.</p>
                </div>
                <a href="{{ route('admin.jenissurat.create') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div class="bg-gray-800 p-5 rounded-xl border-l-4 border-yellow-400 shadow">
                    <h3 class="text-sm text-gray-400">Total Jenis Surat</h3>
                    <p class="text-3xl font-bold mt-2">{{ $surat->count() }}</p>
                </div>
                <div class="bg-gray-800 p-5 rounded-xl border-l-4 border-green-500 shadow">
                    <h3 class="text-sm text-gray-400">Jenis Aktif</h3>
                    <p class="text-3xl font-bold mt-2">{{ $surat->where('is_active', true)->count() }}</p>
                </div>
                <div class="bg-gray-800 p-5 rounded-xl border-l-4 border-red-500 shadow">
                    <h3 class="text-sm text-gray-400">Jenis Nonaktif</h3>
                    <p class="text-3xl font-bold mt-2">{{ $surat->where('is_active', false)->count() }}</p>
                </div>
            ...
            {{-- FILTER --}}
            <form method="GET" action="{{ route('admin.jenissurat.index') }}"
                class="flex flex-wrap items-center justify-between mb-6 gap-4">
                <select name="status"
                    class="border border-gray-700 bg-gray-800 text-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-yellow-400 focus:outline-none w-40">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <button type="submit"
                        class="bg-yellow-400 text-gray-900 px-4 py-2 rounded-lg font-semibold hover:bg-yellow-300 transition">
                        Cari
                    </button>
                    <a href="{{ route('admin.jenissurat.index') }}"
                        class="bg-gray-700 text-gray-300 px-3 py-2 rounded-lg hover:bg-gray-600 transition">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <table class="w-full text-sm text-left text-gray-200">
                    <thead class="bg-gray-800 text-yellow-400 uppercase text-xs font-semibold">
                        <tr>
                            ...
                            <th class="px-5 py-4">Nama Surat</th>
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @forelse ($surat as $key => $item)
                            <tr class="hover:bg-gray-700 transition">
                                ...
                                <td class="px-5 py-4 font-medium text-gray-100">{{ $item->nama_surat }}</td>
                                <td class="px-5 py-4 text-gray-400">{{ $item->deskripsi ?? '-' }}</td>
                                <td class="px-5 py-4">
                                    @if ($item->is_active)
                                        <span
                                            class="bg-green-600 text-white px-3 py-1 rounded-full text-xs font-medium">Aktif</span>
                                    @else
                                        <span
                                            class="bg-gray-600 text-gray-200 px-3 py-1 rounded-full text-xs font-medium">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="px-5 py-4">
                                    @if ($item->template_path)
                                        <a href="{{ asset('storage/' . $item->template_path) }}" target="_blank"
                                            class="text-yellow-400 hover:underline">Lihat Template</a>
                                    @else
                                ...
                                <td class="px-5 py-4 text-center">
                                    <div class="flex justify-center gap-4 text-lg">
                                        <a href="{{ route('admin.jenissurat.show', $item->id) }}"
                                            class="text-blue-400 hover:text-blue-500" title="Lihat">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.jenissurat.edit', $item->id) }}"
                                            class="text-yellow-400 hover:text-yellow-300" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{ route('admin.jenissurat.destroy', $item->id) }}"
                                            method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-gray-400">Belum ada jenis surat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
